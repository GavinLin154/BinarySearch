...
public int binarySearch(int catNumToFind)
{
  ...
  while(minIndex <= maxIndex) {
    if(store[(minIndex + maxIndex) / 2].getCatNum() == catNumToFind) {
      return store[(minIndex + maxIndex) / 2].getInventory();
    } else if(store[(minIndex + maxIndex) / 2].getCatNum() > catNumToFind) {
